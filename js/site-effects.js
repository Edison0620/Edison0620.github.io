...
(function() {
  ...
  class SiteEffectsController {
    ...
    _codeTarget(target) {
      const code = target.closest('.post-body .highlight, .post-body pre');
      return code?.closest('.post-body .highlight') || code;
    }
    ...
    _effectLink(link, event, kind) {
      ...
      if (kind === 'card') {
        ...
        let animation;
        try {
          animation = this.runtime.scatterTags(tags, link, {
            mobile: this.coarsePointer.matches
          });
        } catch (error) {
          this._warn(error);
    ...
    _onDoubleClick(event) {
      const code = this._codeTarget(event.target);
